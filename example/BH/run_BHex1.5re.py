...
OUTPUT_DIR = PROJECT_ROOT / "outputs" /"BH1.5re"/ "BH Full operator"
...
def _reference_2_1_delta_energy(cfg, nroots_per_component=12, tol=1e-6, singlet_s2_tol=1e-3):
    """Return BH 2^1Delta reference from C2v (A1 + A2) singlet subspace.
...
def _parse_args():
    parser = argparse.ArgumentParser(
        description="Run BH workflow from a single script."
    )
    parser.add_argument(
        "mode",
        nargs="?",
...
def main():
    ...
    if run_qsceom:
        ...
        print(
            "BH 2^1Delta energy difference (Excited - Ground):",
